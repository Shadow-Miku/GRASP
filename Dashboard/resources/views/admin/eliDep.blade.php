...
@section('contenido')
    ...
    <div class="container col-md-6 mb-5"> 
        ...
        <div class="card text-center">
            ...
            <div class="card-body">
                <p class="card-text"> Nombre: {{$consultaId->departamento}} </p>
                <p class="card-text"> Creado el: {{$consultaId->created_at}} </p>
                
            </div>
            <form method="POST" action="{{route('adminDep.destroy', $consultaId->idDep)}}">
                @csrf
                @method('delete')
            <div class="card-footer text-muted">
            <button type="submit" class="btn btn-primary  m-1"> Si, eliminalo </button>
            <a href="{{route('adminDep.index')}}" class="btn btn-warning">No, Regresa a admin </a>
            </div>
            </form>       
        </div>
    </div>
